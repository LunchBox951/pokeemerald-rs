...
**7. Self-explanatory code (`self-explanatory-code`).** Comments explain what the code cannot. Make code carry its mechanics through clear names and structure. Move a comment's meaning into names, structure, or tests; what none of those can hold, such as an upstream-owned contract, a comment states in one or two lines; a `// SAFETY:` block and the derivation that principle 2 routes beside its implementation take the length they need. Never narrate mechanics, restate names, or retain implementation history.
