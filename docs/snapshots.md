...
Failed captures retain their staging or unpublished generation directory and report its last known path. Inspect retained directories before removing them manually: another process may have moved or replaced an entry. Automatic directory cleanup cannot make deletion conditional on the identity held by the capture. Linux and macOS promotion atomically refuse an occupied generation name.
