...
/// rejection that already advanced the draw is a genuine mid-turn failure (for example an
/// opponent forced into an unsupported Struggle) and is returned immediately instead of
/// being retried. A Pokémon whose every slot is spent exhausts the loop and returns the
/// last rejection unchanged; that forced-Struggle case is left to #877.
