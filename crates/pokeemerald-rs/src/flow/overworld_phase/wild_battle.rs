...
impl OverworldPhase {
    ...
    /// roll outright (no draws, no bookkeeping) -- the one fail-closed screen
    /// left on this path, and the same no-draw-at-all shape the fainted-lead
    /// guard used to apply before issue #261's white-out retired it
