...
mod tests {
    ...
    /// The 412 `gSpeciesNames` entries of `src/data/text/species_names.h:2-413`
    /// in table order, transcribed independently of [`NAMES`]. The sampled
    /// checks above give direct diagnostics on the notable rows; this pins
    /// every remaining spelling and punctuation mark.
    #[rustfmt::skip]
    const UPSTREAM_SPECIES_NAMES: [&str; 412] = [
        ...
        "SANDSLASH", "NIDORAN\u{2640}", "NIDORINA", "NIDOQUEEN",
        "NIDORAN\u{2642}", "NIDORINO", "NIDOKING", "CLEFAIRY",
    ...
    #[test]
    fn every_display_name_matches_the_upstream_table_entry_for_entry() {
        assert_eq!(
            NAMES.len(),
            UPSTREAM_SPECIES_NAMES.len(),
            "one entry per upstream `gSpeciesNames` row"
        );
        for (index, (&got, &want)) in NAMES.iter().zip(UPSTREAM_SPECIES_NAMES.iter()).enumerate() {
            assert_eq!(
                got, want,
                "NAMES[{index}] is {got:?}, but upstream row {index} is {want:?}"
            );
        }
    }
}
