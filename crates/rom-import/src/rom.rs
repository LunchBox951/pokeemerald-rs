...
impl Rom {
    ...
    pub fn load(path: &Path) -> Result<Self, ImportError> {
        // Check the size from metadata before reading, so pointing the
        // importer at a huge file fails instead of filling memory with it.
        ...
        let bytes = fs::read(path).map_err(|source| ImportError::ReadFailed {
            path: path.to_path_buf(),
            source,
        })?;
