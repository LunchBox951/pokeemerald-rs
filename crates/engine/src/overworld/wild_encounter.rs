...
//! grass:
//!
//! 1. `GetCurrentMapWildMonHeaderId` (`:296`) — a table lookup, no draw. Here
//!    the caller passes the already-resolved [`WildEncounterHeader`].
//! 2. `MetatileBehavior_IsLandWildEncounter(curMetatileBehavior)` (`:596`) —
//!    no draw. [`super::metatile_behavior::is_land_wild_encounter`].
//! 3. `landMonsInfo == NULL` (`:598`) — no draw.
...
//!    only once a roamer is actually on this map (`roamer.c`'s `IsRoamerAt`
//!    gate precedes its `Random() % 100`), and no roamer exists until a
//!    legendary is released.
//! 7. `DoMassOutbreakEncounterTest()` (`:612`, `:479-489`) — no draw on a
...
//!   (`:308-315`): header resolution is the caller's, and
...
/// `ChooseWildMonLevel`'s arithmetic (`wild_encounter.c:268-297`) without its
/// draw.
///
/// Upstream orders the two bounds first ("make sure minimum level is less
/// than maximum level", `:277-286`), so an inverted table entry still yields a
...
///
/// The Hustle / Vital Spirit / Pressure branch that follows (`:293-303`,
...
/// `ChooseWildMonLevel` (`wild_encounter.c:268-297`): one `Random()` draw
...
/// map (`GetCurrentMapWildMonHeaderId`, `:296-330`); `None` models
...
pub fn standard_wild_encounter(
    ...
    // `MetatileBehavior_IsLandWildEncounter` (`:596`) — both before any draw,
    // so an ordinary step on ordinary ground costs nothing.
    let header = header?;
    if !is_land_wild_encounter(cur_metatile_behavior) {
        return None;
    }
    // `gWildMonHeaders[headerId].landMonsInfo == NULL` (`:598`): a map with a
