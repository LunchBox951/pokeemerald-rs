...
//! spent and Struggle is forced), the escape roll when the player runs,
//! then each resolved hit's own draws as `battle::hit` pins them. No
//! matchup below ties on Speed.
