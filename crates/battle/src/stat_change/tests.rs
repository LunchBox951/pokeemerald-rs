...
//! two tails' differing draw counts, and the cap/floor outcomes.

use super::{
    ensure_resolvable, is_stat_change_effect, resolve_stat_change_move, stat_change_for_effect,
    ChangedStat, StatChangeDirection, StatChangeOutcome, EFFECT_ATTACK_DOWN, EFFECT_DEFENSE_DOWN,
    STAT_CHANGE_EFFECTS,
};
...
/// Sand Attack reaches a stage no pre-#293 effect could name at all: the
/// target's **accuracy**.
#[test]
