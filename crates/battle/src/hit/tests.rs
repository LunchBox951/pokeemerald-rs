//! [`crate::hit`]'s own unit tests: the exact draw order and count of
//! `BattleScript_EffectHit`, the hand-computed damage pins, and the
//! allow-list boundary.
...
fn a_high_crit_move_crits_on_a_draw_a_plain_move_would_not() {
    ...
    // `resolve_hit` feeds the move's effect into `crit_stage`
