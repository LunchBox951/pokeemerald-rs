...
- Inspect the exact upstream artifact with `scripts/ledger.py inspect` or a focused `gaps` query for every behaviour or asset change. Ledger `spec` values are current v1 acceptance IDs such as `S-3`, never legacy domain labels. Update only the evidence proven by the change; never infer a neighbouring or parent disposition or hand-edit `ledger/pokeemerald.json`.
